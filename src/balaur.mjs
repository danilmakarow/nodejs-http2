...
export default class Daemon {
  static get EXIT_CODE_OK() {
    return 0;
  }

  static get EXIT_CODE_CATCH_ALL() {
    return 1;
  }

  static get EXIT_CODE_CANNOT_EXECUTE() {
    return 126;
  }

  static get EXIT_CODE_CONTROL_C() {
    return 130;
  }

  static get EXIT_CODE_UNHANDLED_REJECTION() {
    return 131;
  }

  static get EXIT_CODE_UNHANDLED_EXCEPTION() {
    return 132;
  }

  static get EXIT_CODE_TERMINATED() {
    return 255;
  }
  ...
  #start() {
    ...
    if (process.pid === this.#pid) {
      ...
      if (!this.#processExists()) {
        ...
        console.warn(
          "The pidFile contained an old id. Did the previous process die?",
        );
        this.#startDaemon();
      } else {
        if (cluster.isPrimary) {
          console.error(`Daemon already started, unable to start. Please use stop & start or restart.`);
  ...
  #restart() {
    ...
    if (!this.#processExists()) {
      ...
      process.kill(
        this.#pid,
        process.platform.startsWith("win") ? "SIGINT" : "SIGHUP",
      );
  ...
  #startSequence() {
    if (cluster.isPrimary) {
      ...
      cluster.on("exit", (worker, code, signal) => {
        console.info(
          `Cluster worker with PID ${worker.process.pid} exited with code ${code} and signal ${signal}.`,
        );
        ...
        if (exitCount++ > 10) {
          console.error(
            "Cluster worker exited too many times, unable to fork any further.",
          );
          return;
        }
        ...
        if (code === self(this).EXIT_CODE_UNHANDLED_REJECTION) {
          console.error(
            `Cluster worker exited with code ${self(this).EXIT_CODE_UNHANDLED_REJECTION} (unhandled rejection), unable to fork any further.`,
          );
          return;
        }
        ...
        if (code === self(this).EXIT_CODE_UNHANDLED_EXCEPTION) {
          console.error(
            `Cluster worker exited with code ${self(this).EXIT_CODE_UNHANDLED_EXCEPTION} (unhandled exception), unable to fork any further.`,
          );
          return;
        }
      ...
      process.on("SIGHUP", () => {
        console.info(
          "Process received SIGHUP, restarting cluster cluster workers.",
        );
        cluster.disconnect();
        ...
        for (const worker of clusterWorkers) {
          ...
          console.info(
            `Cluster worker (PID ${worker.process.pid}) killed with SIGTERM.`,
          );
        }
      ...
      process.on("SIGINT", () => {
        ...
        console.info(
          "Active connection may or may not be stopped. Use --force to ensure.",
        );
        cluster.disconnect();
        process.exit();
      });
      ...
      process.on("SIGTERM", () => {
        console.info(
          "Process received SIGTERM, disconnecting cluster workers.",
        );
        cluster.disconnect();
